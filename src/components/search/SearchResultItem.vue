  ...
  >
    <div class="flex items-start space-x-3 sm:space-x-4">
      ...
      <div class="flex-shrink-0">
        <LazyOptimizedPicture
          :src="post.featuredImage?.src || 'all-things-digital/thumbnail-01-comp'"
          :alt="post.featuredImage?.alt || post.title"
          :width="64"
          :height="64"
          img-class="w-12 h-12 sm:w-16 sm:h-16 rounded-lg object-cover"
      ...
      <div class="flex-1 min-w-0">
        ...
        <h3
          ...
          v-html="post._highlightedTitle || post.title"></h3>

        <!-- Excerpt/Snippet -->
        <p class="mt-1 text-xs sm:text-sm text-gray-600 dark:text-gray-300 line-clamp-2 leading-relaxed"
          v-html="post._highlightedExcerpt || post._snippet || post.excerpt || 'No description available'"></p>
        ...
        <div class="mt-2 sm:mt-3 flex flex-wrap items-center gap-2 sm:gap-4 text-xs text-gray-500 dark:text-gray-400">
          ...
          <span
            ...
            v-html="post._highlightedCategory || post.category"></span>
        ...
        <div v-if=" post.tags && post.tags.length > 0 " class="mt-2 flex flex-wrap gap-1">
          <span v-for=" ( tag, index ) in post._highlightedTags || post.tags.slice( 0, isMobile ? 2 : 3 ) " :key="index"
            ...
            v-html="typeof tag === 'string' ? `#${tag}` : `#${tag}`"></span>
          <span v-if=" post.tags.length > ( isMobile ? 2 : 3 ) "
    ...
    <!-- Selection Indicator -->
    <div v-if=" isSelected " class="absolute left-0 top-0 bottom-0 w-1 bg-indigo-500 rounded-r"></div>
  </div>
</template>
