<template>
  ...
  <article
    ...
    aria-labelledby="article-title article-excerpt" itemtype="https://schema.org/BlogPosting">
    <!-- Featured Image with proper figure semantics -->
    <figure class="mb-0 shrink-0 md:w-2/5" itemprop="image" itemscope itemtype="https://schema.org/ImageObject">
      <LazyOptimizedPicture :src="imageSrc" :alt="imageAlt" :width="1000" :height="600"
    ...
    <section class="flex grow flex-col p-4 md:p-5 md:ml-0" role="region" aria-labelledby="article-title">
      ...
      <header class="mb-3 flex items-center">
        <AvatarAuthor :imageSrc="authorImageSrc" :imageAlt="authorImageAlt" :link="authorLink" itemprop="author"
          itemscope itemtype="https://schema.org/Person" />
        <section id="postMetaData" class="grow">
          <!-- Main article title as h2 for proper document outline -->
          <h2 id="article-title" class="text-left font-navigation text-base text-slate-700 line-clamp-2 dark:text-white"
            itemprop="headline">
          ...
          <section class="flex items-center text-xs text-slate-500 dark:text-gray-400 mt-1">
            <span v-if=" authorName " class="flex items-center text-nowrap">
              <span itemprop="author" class="hidden sm:hidden md:flex md:mr-1 md:mr-2">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                  stroke="currentColor" class="mr-1.5 h-3.5 w-3.5" aria-hidden="true" focusable="false" role="img">
                  <title>Author</title>
              ...
              <span itemprop="category" class="mr-1 md:mr-2">
                <router-link :to="`/blog/category/${category.toLowerCase()}`"
                  class="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors" itemprop="articleSection">
                  {{ category }}
                </router-link>
              </span>
            </span>
      ...
      <footer class="flex items-center text-xs text-slate-500 mt-2 dark:text-gray-400" aria-label="Article tags">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
          class="mr-2 h-5 w-5" aria-hidden="true" focusable="false" role="img">
          <title>Tags</title>
          <path stroke-linecap="round" stroke-linejoin="round"
            d="M9.568 3H5.25A2.25 2.25 0 003 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 005.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 009.568 3z" />
          <path stroke-linecap="round" stroke-linejoin="round" d="M6 6h.008v.008H6V6z" />
        ...
        <span itemprop="keywords">
          <template v-for=" ( tag, index ) in tags " :key="tag">
            <router-link :to="`/blog/tag/${getTagSlug( tag )}`"
              class="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"
              :aria-label="`Browse articles tagged ${tag}`" itemprop="about">
              {{ tag }}
            </router-link>
...
import LazyOptimizedPicture from '../common/LazyOptimizedPicture.vue';
