<template>
  <section id="homeBlogArticles" class="flex flex-col">
    ...
    <div v-if="limitedPosts.length === 0" class="text-center text-gray-500 dark:text-gray-400 py-10">
...
const limitedPosts = computed(() => {
  ...
  return postsData.filter(post => post.status === 'published' || !post.status).slice(0, 3);
});
</script>
