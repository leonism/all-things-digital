  ...
  <section id="mainWrapper" class="max-w-4xl mx-auto" role="main">
    ...
    <section class="mb-12 text-center relative">
      ...
      <div class="mt-6 flex flex-wrap justify-center gap-4 sm:gap-6">
        <div
          class="bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm rounded-2xl px-6 py-3 border border-gray-200/50 dark:border-gray-700/50 shadow-lg"
        >
    ...
    >
      <div class="space-y-6">
        <div
          ...
          class="group relative overflow-hidden transition-all duration-500 bg-gradient-to-r from-white/80 to-white/60 dark:from-gray-800/80 dark:to-gray-800/60 backdrop-blur-sm rounded-2xl border border-gray-200/60 dark:border-gray-700/60 hover:shadow-2xl hover:scale-[1.01] hover:-translate-y-1"
          :class="[
        ...
        >
          ...
          <div
            class="relative p-6 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6"
          >
            <!-- Left section: Category header and posts preview -->
            <div class="flex-1 lg:flex lg:items-center lg:gap-8">
              <!-- Category header with icon -->
              <div
                class="flex items-center gap-3 mb-4 lg:mb-0 lg:min-w-0 lg:flex-shrink-0"
              >
                <div
                ...
                <div>
                  <h3
                    class="text-lg font-bold text-gray-900 dark:text-white group-hover:text-gray-700 dark:group-hover:text-gray-200 transition-colors duration-300"
                  >
                    {{ category }}
                  </h3>
                  <div
                    class="inline-flex px-2 py-1 rounded-full text-xs font-semibold transition-colors duration-300 mt-1"
                    :class="[
                      index % 4 === 0
              ...
              <div class="flex-1 lg:ml-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div
                    v-for="(post, postIndex) in getLatestPostsByCategory(
                      category,
                    ).slice(0, 2)"
                    :key="post.slug"
                  ...
                  >
                    <div class="flex items-start space-x-3">
                      <div class="relative">
                        ...
                        <div
                          v-else
                          class="w-12 h-12 bg-gradient-to-br from-gray-200 to-gray-300 dark:from-gray-600 dark:to-gray-700 rounded-lg flex items-center justify-center"
                        >
                          <svg
                      ...
                      <div class="flex-1 min-w-0">
                        ...
                        <h4
                          class="text-sm font-semibold text-gray-900 dark:text-white truncate group-hover/post:text-indigo-600 dark:group-hover/post:text-indigo-400 transition-colors duration-300"
                        >
                          {{ post.title }}
                        </h4>
                        <p
                ...
                <div
                  v-if="getLatestPostsByCategory(category).length > 2"
                ...
                >
                  ...
                  >
                    ...
                    {{ getLatestPostsByCategory(category).length - 2 }} more
                    posts
                  </div>
                </div>
              </div>
            ...
            <div class="lg:flex-shrink-0">
              <button
                ...
                class="w-full lg:w-auto group/btn relative overflow-hidden px-6 py-3 text-sm font-semibold transition-all duration-300 rounded-xl border-2 hover:scale-[1.02] active:scale-[0.98]"
                :class="[
                  index % 4 === 0
                    ? 'text-indigo-600 dark:text-indigo-400 border-indigo-200 dark:border-indigo-800 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:border-indigo-300 dark:hover:border-indigo-700'
                    : '',
                  index % 4 === 1
                    ? 'text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 hover:border-emerald-300 dark:hover:border-emerald-700'
                    : '',
                  index % 4 === 2
                    ? 'text-purple-600 dark:text-purple-400 border-purple-200 dark:border-purple-800 hover:bg-purple-50 dark:hover:bg-purple-900/30 hover:border-purple-300 dark:hover:border-purple-700'
                    : '',
                  index % 4 === 3
                    ? 'text-orange-600 dark:text-orange-400 border-orange-200 dark:border-orange-800 hover:bg-orange-50 dark:hover:bg-orange-900/30 hover:border-orange-300 dark:hover:border-orange-700'
                    : '',
                ]"
              >
                <span
    ...
    >
      ...
      <div class="absolute inset-0 -z-10">
        <div
          class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-gradient-to-br from-indigo-100/30 to-purple-100/30 dark:from-indigo-900/20 dark:to-purple-900/20 rounded-full blur-3xl"
        ></div>
      </div>
      ...
      <div class="mb-6">
        <div
          class="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 rounded-2xl"
        >
          <svg
      ...
      <p class="mb-6 text-lg text-gray-600 dark:text-gray-300 max-w-md mx-auto">
        <span v-if="categoryParam">
          ...
          <strong>{{ displayCategoryName }}</strong> category yet.
        </span>
        <span v-else> No posts are available at the moment. </span>
      </p>
...
const allCategories = computed(() => {
  ...
  (blogData as Post[]).forEach((post: Post) => {
    if (post.category) categories.add(post.category);
  });

  // Sort categories by latest post date
  return Array.from(categories).sort((a: string, b: string) => {
    const latestPostA = getLatestPostsByCategory(a)[0];
    const latestPostB = getLatestPostsByCategory(b)[0];

    if (!latestPostA && !latestPostB) return 0;
    if (!latestPostA) return 1;
    if (!latestPostB) return -1;
...
function getLatestPostsByCategory(category: string): Post[] {
  return (blogData as Post[])
...
onMounted(() => {
  ...
  const filtered = (blogData as Post[]).filter(
    (post: Post) =>
