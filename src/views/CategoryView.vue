  ...
  <section id="mainWrapper" class="max-w-4xl mx-5 sm:mx-5 md:mx-10 lg:mx-auto" role="main">
    ...
    <section class="pb-8">
      ...
      <div v-if=" categoryParam && categoryPosts.length > 0 " class="space-y-8">
        <BlogPostCard v-for=" post in categoryPosts " :key="post.slug" :imageSrc="post.featuredImage.src"
          :imageAlt="post.featuredImage.alt || post.title" :title="post.title" :postLink="`/category/${post.slug}`"
      ...
      <div v-else-if=" !categoryParam " class="space-y-8">
        <article v-for=" ( category, index ) in sortedCategories " :key="category"
          ...
          <section class="px-6 pb-4">
            <div class="space-y-3">
              <div v-for=" post in getLatestPosts( category, 2 ) " :key="post.slug"
                class="bg-slate-50 dark:bg-gray-700/50 rounded-lg p-3 hover:bg-slate-100 dark:hover:bg-gray-700 transition-colors">
                <h4 class="text-slate-700 dark:text-white font-medium text-xl line-clamp-1">
                  {{ post.title }}
                </h4>
                <p class="text-slate-500 dark:text-gray-400 text-xs mt-1">
                  {{ formatDate( post.date ) }}
                </p>
                <p class="text-slate-600 prose dark:text-gray-300 text-md line-clamp-2 mt-1">
                  {{ post.excerpt }}
                </p>
              </div>
            </div>
      ...
      <div v-else class="text-center py-16">
        <div class="max-w-md mx-auto">
          <svg class="mx-auto h-12 w-12 text-slate-400 dark:text-gray-500" fill="none" viewBox="0 0 24 24"
...
const baseUrl = 'https://all-things-digital.pages.dev/'; // Replace with your base URL
