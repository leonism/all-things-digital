...
const filteredPosts = computed(() => {
  ...
  return postsData.filter(post =>
    ...
    post.categories && post.categories.map(c => c.toLowerCase()).includes(lowerCaseCategory)
  );
});

// Computed properties for meta tags
const pageTitle = computed(() => `Category: ${categoryName.value || 'Archive'} | DGPond.COM`); // Updated title
const pageDescription = computed(() => `Posts categorized under ${categoryName.value || 'archive'}.`);
const canonicalUrl = computed(() => {
    const base = 'https://yourdomain.com'; // <<<--- IMPORTANT: Replace with your actual domain
    return categoryName.value ? `${base}/blog/category/${encodeURIComponent(categoryName.value)}` : `${base}/blog`;
});
